		...
		BEGIN
			...
			BEGIN
				IF @P_PAREDE_ATUAL <> @P_PAREDE BEGIN
					...
					INSERT INTO #TEMP_REQ(MATRICULA, REQUISICAO) 
						SELECT RF.MATRICULA, R.REQUISICAO
						FROM REQUISICAO R
						JOIN REQFUNCAO RF ON RF.REQUISICAO = R.REQUISICAO
					...
					INSERT INTO #TEMP_BLOQUEIO(MATRICULA)
						SELECT B.MATRICULA FROM BLOQUEADO B
							...
							INNER JOIN REQUISICAO R ON R.REQUISICAO = RC.REQUISICAO
							WHERE R.DIA = @P_DATA AND R.PERIODO = @P_PAREDE
							GROUP BY RC.CARGA);
				...
				BEGIN					
					...
					BEGIN
						...
						BEGIN
							...
							SET @P_COUNT_NAOFEZ11H =  @P_COUNT_NAOFEZ11H + 1;
						END
						ELSE IF @P_BLOQUEADO > 0 BEGIN
							PRINT 'ELSE IF BLOQUEADO'
							SET @P_COUNT_BLOQUEADO = @P_COUNT_BLOQUEADO + 1;
						END
						ELSE 
						BEGIN
				...
				IF @P_COUNT_TEMOCORRENCIA > 0
					UPDATE ASSIDUIDADE SET SITUACAO = 0, OBSERVACAO = 'TEM OCORRÊNCIA : ' + @P_OCORRENCIA_DCR WHERE MATRICULA = @MATRICULA AND PAREDE = @P_PAREDE AND DATA = @P_DATA;
