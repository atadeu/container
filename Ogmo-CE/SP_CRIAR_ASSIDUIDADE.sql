...
ALTER PROCEDURE SP_CRIAR_ASSIDUIDADE 
AS
...
BEGIN
	...
	*/
	...
	DECLARE @P_DATA DATETIME, @JA_INSERIU INT, @P_DATAFIM DATETIME, @TEM_ASSIDUIDADE INT,  @MATRICULA INT, @P_PAREDE INT, @REQUISICAO INT, @FUNCAO INT, @DIA DATE;
	--SET @P_ANO = 2015;
	--SET @P_MES = 5;
	...
	DECLARE C_RODA CURSOR FOR
		SELECT MES, ANO FROM RODA_ASSIDUIDADE
			WHERE RODA = 1;
	
	DECLARE @P_MES INT, @P_ANO INT;
	
	OPEN C_RODA;
	FETCH NEXT FROM C_RODA INTO @P_MES, @P_ANO;
	...
	BEGIN
		...
		BEGIN CATCH
		   ...
		   --Insert Into [LOG] (auto, login, tabela, campo, descricao) Values ((select MAX(auto)+1 From log), 'ASSIDUIDADE', 'SP', 'ERRO', substring(convert(varchar, ERROR_NUMBER()) + ' ' + ERROR_MESSAGE(),1,500));
		   --End
		END CATCH;
	...
	UPDATE RODA_ASSIDUIDADE SET RODA = 0;
END